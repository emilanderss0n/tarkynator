...
foreach ($data as $key => &$value) {
    ...
    $idMapping[$key] = $newId;
    $tplMapping[$value['item']['_id']] = $newId; // Map old _id to new _id for _tpl updates
    // Update the main item _id and add a new MongoDB-like ID to each slot within Slots, if they exist
    if (isset($value['item']['_props']['Slots']) && is_array($value['item']['_props']['Slots'])) {
        foreach ($value['item']['_props']['Slots'] as &$slot) {
...
file_put_contents($assortOutputFile, json_encode($assortData, JSON_PRETTY_PRINT));
