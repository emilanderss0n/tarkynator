  ...
  const updateHandbookContent = async (itemElement, usedInTasksHTML) => {
    try {
                ...
                const slotItems = slot._props.filters.flatMap((filter) =>
                  filter.Filter
                    ? filter.Filter.map((filterId) => {
                        ...
                        const itemId =
                          itemsData && itemsData[filterId]
                            ? itemsData[filterId]._id
                            : null;
      ...
      const image512pxLink = itemData.image512pxLink
        ? itemData.image512pxLink.replace(/^.*\/data\/images\//, "data/images/")
        : "";
      ...
      try {
        ...
        handbookContent.innerHTML = `
          ...
          itemData.shortName
        } / Weight: ${itemData.weight} / Base Price: <span class="global-id">${
          itemData.basePrice
        }</span>
                                        </figcaption>
                                    </figure>
