...
import { QUESTS_URL } from './localData.js';
        ...
        fetchData(url, options)
            .then(data => {
                if (data && data.data && data.data.tasks) {
                    localStorage.setItem('questsData', JSON.stringify(data.data.tasks));
                    const activeCategory = getActiveQuestCategory();
                    const selectedMap = selectMap.value;
                    const availableMaps = new Set();
                    const filteredQuests = data.data.tasks.filter(quest => {
                        ...
                        <div class="quest-item card" data-quest-id="${quest.id}" data-quest-map="${quest.map ? quest.map.name : 'Any'}">
                            <div class="card-body">
                                <a href="javascript:void(0);" class="image">
                                    <img src="${quest.taskImageLink}" alt="${quest.name}">
                                </a>
                                <div class="details">
